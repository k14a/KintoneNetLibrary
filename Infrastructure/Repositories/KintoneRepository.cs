...
public class KintoneRepository : IKintoneRepository {
    ...
    private KintoneApi ResolveApi(KintoneModelBase model) {
        return _factory.Create(model);
    }

    public Task<string> CreateRecordsAsync<T>(IList<T> records) where T : KintoneModelBase {
        return ExecuteCudAsync(records, (api, json) => api.CreateAsync(json), KintoneRequestBuilder.BuildCreateJson);
    }
    public Task<string> UpdateRecordsAsync<T>(IList<T> records) where T : KintoneModelBase {
        return ExecuteCudAsync(records, (api, json) => api.UpdateAsync<T>(json), KintoneRequestBuilder.BuildUpdateJson);
    }
    public Task<string> DeleteRecordsAsync<T>(IList<T> records) where T : KintoneModelBase {
        return ExecuteCudAsync(records, (api, json) => api.DeleteAsync(json), KintoneRequestBuilder.BuildDeleteJson);
    }
    public Task<string?> FindByIDAsync<T>(T model, string id) where T : KintoneModelBase, new() {
    ...
    public Task<string?> FindAllAsync<T>(T model) where T : KintoneModelBase, new() {
        return ExecuteFindAsync<T>(model, api => api.FindAllAsync<T>());
    }

    public Task<string?> FindByFieldAsync<T>(T model, string field, string value) where T : KintoneModelBase, new() {
        return ExecuteFindAsync<T>(model, api => api.FindByFieldAsync<T>(field, value));
    }

    public Task<string?> FindByQueryAsync<T>(T model, string queryStr) where T : KintoneModelBase, new() {
